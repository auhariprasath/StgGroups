...
import { specFieldsFor, companyBaseFieldsFor } from "@/lib/data/specs";
...
const FIELDS: FieldDef[] = [
  { key: "location", label: "Site Location", tab: 0, placeholder: "Project site city / area", hint: "Where the machine will actually work." },
  { key: "duration", label: "Duration", tab: 0, placeholder: "e.g. 30 days" },
  { key: "workingHeight", label: "Working Height", tab: 0, placeholder: "e.g. 22 metres / 60 feet" },
  { key: "deliveryDate", label: "Delivery Date", tab: 0, type: "date", hint: "Schedules the delivery task + reminders." },
  { key: "companyName", label: "Company Name", tab: 1, placeholder: "Client company" },
  { key: "companyAddress", label: "Company Address", tab: 1, placeholder: "Registered / billing address" },
  ...
  { key: "email", label: "Company Email", tab: 1, type: "email", placeholder: "name@company.com", hint: "Quotation is auto-mailed here." },
  { key: "landline", label: "Landline", tab: 1, placeholder: "With STD code" },
  { key: "deliveryAddress", label: "Delivery Address", tab: 2, placeholder: "Exact site delivery address", hint: "May differ from billing — used on the quotation." },
  { key: "deliveryGstin", label: "Delivery GSTIN", tab: 2, placeholder: "Site-state GSTIN if different" },
...
function RequirementForm() {
  ...
  const initial = useMemo<Values>(() => {
    ...
    const v: Values = {};
    // Build initial values from all possible field keys (FIELDS + spec keys from saved requirement)
  ...
  const [categoryId, setCategoryId] = useState<string | null>(existing?.categoryId ?? lead?.categoryId ?? null);
  ...
  const categories = db.categories.filter((c) => c.companyId === lead.companyId);
  const specDefs: FieldDef[] = specFieldsFor(categoryId).map((s) => ({ key: s.key, label: s.label, tab: 0, placeholder: s.placeholder, hint: s.hint }));
  // Phase 5: company base fields always appear in Tab 0.
  const baseDefs: FieldDef[] = companyBaseFieldsFor(lead.companyId).map((s) => ({ key: s.key, label: s.label, tab: 0, placeholder: s.placeholder, hint: s.hint }));
  const allFields: FieldDef[] = [...specDefs, ...baseDefs, ...FIELDS];

  const setVal = (k: string, value: string) => setValues((v) => ({ ...v, [k]: { ...v[k], value } }));
  const setNil = (k: string, nilReason: string) => setValues((v) => ({ ...v, [k]: { ...v[k], nilReason } }));
  ...
  const handleEnter = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      ...
      else toast.error("Fill all fields on this tab (use 'nil' + a reason if you don't have the data)");
    }
  };
  ...
  const save = () => {
    ...
    const fields: RequirementField[] = allFields.map((f) => ({
      ...
      nilReason: values[f.key].value.trim().toLowerCase() === "nil" ? values[f.key].nilReason : undefined,
    }));
    ...
    mutate((d) => {
      ...
      const base = {
        ...
        fields,
        deliveryDate: deliveryDate && deliveryDate.toLowerCase() !== "nil" ? new Date(deliveryDate).toISOString() : undefined,
      };
      if (idx >= 0) {
      ...
      if (l && (l.status === "new" || l.status === "followup")) l.status = "interested";
      d.activities.push({
        id: newId("a"), leadId, at: new Date().toISOString(), byUserId: user?.id ?? "u-md",
        kind: "requirement", text: existing ? "Requirement updated" : "Requirement gathered — ready to quote",
      });
    });
    clear();
  ...
  return (
    <>
      <Link to="/leads/$leadId" params={{ leadId }} className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="rounded-2xl border bg-card p-5 shadow-card sm:p-6">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{companyName(db, lead.companyId)} · all fields mandatory</p>
          </div>
          <span className="text-xs text-muted-foreground">{savedAt ? `Autosaved ${savedAt.toLocaleTimeString()}` : "Autosaves as you type"}</span>
        </div>

        {/* Equipment category drives the specification fields below */}
        <div className="mb-4 grid gap-1.5 sm:max-w-xs">
          <Label className="flex items-center">Equipment / product <InfoTip tip="Sets the technical spec fields to collect for this product." /></Label>
          ...
          {TABS.map((_, t) => (
            <TabsContent key={t} value={String(t)} className="grid gap-4 sm:grid-cols-2">
              {t === 0 && specDefs.length === 0 && (
                <p className="sm:col-span-2 rounded-lg bg-muted/50 px-3 py-2 text-xs text-muted-foreground">Select the equipment above to collect its technical specs.</p>
              )}
              {allFields.filter((f) => f.tab === t).map((f) => {
                const cell = values[f.key];
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <AlertCircle className="h-3.5 w-3.5" /> Enter moves to the next tab. Nothing is booked until you save.
          </p>
          <div className="flex gap-2">
            {Number(tab) > 0 && <Button variant="outline" onClick={() => setTab(String(Number(tab) - 1))}>Back</Button>}
            {Number(tab) < TABS.length - 1 ? (
              <Button onClick={() => (tabValid(Number(tab)) ? setTab(String(Number(tab) + 1)) : toast.error("Complete this tab first"))}>Next</Button>
            ) : (
              <Button onClick={save} disabled={!allValid}>Save requirement</Button>
            )}
          </div>
        </div>
      </div>
...
function Missing({ text = "Lead not found" }: { text?: string }) {
  return (
    <div className="rounded-xl border bg-card p-10 text-center">
      ...
      <Button asChild variant="outline" className="mt-4"><Link to="/leads">Back to leads</Link></Button>
    </div>
  );
}
